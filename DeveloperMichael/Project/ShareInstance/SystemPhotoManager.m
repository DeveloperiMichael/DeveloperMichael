...
@implementation SystemPhotoManager



+ (instancetype)shareSystemPhotoManagerInstance
{
    static SystemPhotoManager *systemPhotoManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        systemPhotoManager = [[self alloc] init];
    });
    return systemPhotoManager;
}

#pragma mark - 获取所有相册列表
- (NSArray<DMAblumList *> *)getPhotoAblumList
{
    NSMutableArray<DMAblumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if (!([collection.localizedTitle isEqualToString:@"Recently Deleted"] ||
            ...
            if (assets.count > 0) {
                DMAblumList *ablum = [[DMAblumList alloc] init];
                ablum.ablumName = [self transformAblumTitle:collection.localizedTitle];
                ablum.photoNumber = assets.count;
